  ...
  checkColorGroup(e) {
    ...
    if (colorString) {
      ...
      if (isColorInput) {
        ...
        this.setFirstMediaInCurrentGroup();
      }
    }
  }

  setFirstMediaInCurrentGroup(altValue = null) {
    const mediaGalleries = this.productTarget?.querySelector('media-gallery');
    ...
    if (layout === 'thumbnail') {
      ...
      swiper.functionGoto(firstMediaIndex);
      return;
    }

    const galleryViewer = mediaGalleries.querySelector('[id^="GalleryViewer"]');
    if (!galleryViewer) return;

    const galleryItems = Array.from(galleryViewer.querySelectorAll('[data-media-id]'));
    ...
    const firstMediaId = firstMediaItem.getAttribute('data-media-id');
    if (!firstMediaId) return;

    mediaGalleries.setActiveMedia(firstMediaId, false);
    firstMediaItem.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
  ...
  updateMediaGroup(altValue) {
    ...
    if (
      ...
      fetch(`/products/${productHandle}?section_id=media-gallery-slide`)
        ...
        .finally(() => {
          this.setFirstMediaInCurrentGroup(altValue);
          if (!zoomAction) return;
  ...
  customElements.define(
    ...
    class ProductForm extends PopupBase {
      ...
      syncVariantIdBeforeSubmit() {
        ...
        const scope =
          this.closest('.video-item__popup') ||
          this.closest('.product__item-js') ||
          this.closest('.product-detail');
        if (!scope) return;

        const variantPicker = scope.querySelector(
        ...
        if (variantPicker.currentVariant?.id) {
          idInput.value = variantPicker.currentVariant.id;
          return;
        }

        try {
          const variantDataNode = variantPicker.querySelector(
            '[type="application/json"]'
          );
          if (!variantDataNode) return;
          ...
          console.warn('syncVariantIdBeforeSubmit error', error);
        }
      }
...
class ShopableVideo extends PopupBase {
  ...
  onClick(e) {
    // Nếu đang có popup shopable-video mở rồi thì không mở thêm cái mới
    const opened = document.querySelector(
      '.tingle-modal--visible.tingle-modal.shopable-video'
    );
    if (opened) return;
    // (không bắt buộc nhưng nên có) tránh click từ input/label/button bên trong
    if (e && e.target && e.target.closest('input, label, button, select, option, a')) {
      return;
    }
    if (this.popupDiv) {
...
class SlideWithThumbs extends HTMLElement {
  ...
  initThumbnail() {
    ...
    const slidesPerView = this.dataset.thumbSlidesPerView
      ...
      : 4;
    const mobileSlidesPerView = this.dataset.thumbMobileSlidesPerView
      ? this.dataset.thumbMobileSlidesPerView
      : 2;
    ...
    this.thumbnailSlide = new Swiper(container, {
      ...
      slidesPerView: Number(mobileSlidesPerView),
      ...
      breakpoints: {
        768: {
          direction: direction,
          slidesPerView:
            direction == 'vertical' ? 10 : Number(slidesPerView) || 6,
        },
      },
    });
  }
